...
>(storage: Storage) {
  const map = storageMap as Map<Storage, WrappedPromise>;
  ...
  if (!map.has(storage)) {
    map.set(storage, wrapPromise(storage.get()));
  }
  if (_data !== null) {
    map.set(storage, { read: () => _data });
  }

  return _data ?? (map.get(storage)!.read() as Data);
}
